...
func (gen *Generator) Reset(w io.Writer) {
	...
	gen.buf.WriteByte(0x04)
	gen.buf.WriteByte(0x08)
}
...
func (gen *Generator) Bool(b bool) error {
	...
	if b {
		gen.buf.WriteByte(typeTrue)
	} else {
		gen.buf.WriteByte(typeFalse)
	}
...
func (gen *Generator) writeSym(sym string) {
	...
	for i := 0; i < gen.symCount; i++ {
		if gen.symTbl[i] == sym {
			gen.buf.WriteByte(typeSymlink)
			gen.encodeLong(int64(i))
			return
		}
	}

	gen.buf.WriteByte(typeSymbol)
	gen.encodeLong(int64(len(sym)))
	gen.buf.WriteString(sym)
...
func (gen *Generator) Class(name string) error {
	...
	gen.buf.WriteByte(typeClass)
	gen.encodeLong(int64(l))
	gen.buf.WriteString(name)
	return gen.writeAdv()
}
...
func (gen *Generator) StartUserMarshalled(name string) error {
	...
	gen.buf.WriteByte(typeUsrMarshal)
	gen.writeSym(name)
...
func (gen *Generator) UserDefinedObject(name, data string) error {
	...
	gen.buf.WriteByte(typeUsrDef)
	gen.writeSym(name)
	gen.encodeLong(int64(len(data)))
	gen.buf.WriteString(data)
	return gen.writeAdv()
}
...
func (gen *Generator) Regexp(expr string, flags byte) error {
	...
	gen.buf.WriteByte(typeRegExp)
	gen.writeString(expr)
	gen.buf.WriteByte(flags)
	return gen.writeAdv()
}
...
func (gen *Generator) writeAdv() error {
	...
	if l := gen.buf.Len(); l > maxBufferSize || (l > 0 && gen.st.cur.pos == gen.st.cur.cnt && gen.st.sz == 1) {
		if _, err := gen.buf.WriteTo(gen.w); err != nil {
			return err
		}
		gen.buf.Reset()
	}
...
func (gen *Generator) encodeLong(n int64) {
	if n == 0 {
		...
		gen.buf.WriteByte(byte(n + 5))
		return
	} else if -0x7C < n && n < 0 {
		gen.buf.WriteByte(byte((n - 5) & 0xFF))
		return
	}
